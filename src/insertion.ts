...
export function getArticleMaxAds(containerClientHeight, windowInnerHeight, max) {
  if (isNaN(windowInnerHeight)) {
    return 1
  }

  return Math.max(Math.min(Math.ceil(containerClientHeight / windowInnerHeight / 2), max), 1)
}
...
export function getInsertionPositions({ targetContainer, offset, max }) {
  const result = []
  const _max = getArticleMaxAds(targetContainer.clientHeight, window.innerHeight, max)
